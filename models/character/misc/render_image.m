...
%      epsilon=0;
    ...
    if isfield(PM,'mAngle'); 
        rotAngle=PM.mAngle;
    else
        rotAngle=0;
    end
    rotPivot=traj_img{1}(1,:);
    rotation=Modeler.get2DRotateMatrix(rotAngle,rotPivot);
...
%     return;
% end
if any(isnan(lind));return; end
% lind(lind<1)=1;
% lind(lind>(105*105))=105*105;
lind=floor(lind);
for i=1:numel(lind)
