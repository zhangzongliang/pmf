classdef Stroke < BetterHandle
    ...
    properties (GetAccess = private, SetAccess = private)
       ...
       po = {'pos_token','invscales_token','shapes_token','angles_token'}; % tracked properties
    ...
    methods
        ...
        % with the old ones
            ...
            this.angles_token = oldS.angles_token;  
        end
        ...
        % item or from scratch
            ...
            if this.cache_current
                ...
                [motor,motor_spline] = vanillaToMotorWithRotation(...
                    this.shapes_token,this.invscales_token,this.pos_token,this.angles_token);
...
function [motor,motor_spline] = vanillaToMotorWithRotation(vanilla_shapes,invscales,first_pos,angles)
...
%         if 1==i;
%         vanilla_shapes(:,:,i) = invscales(i) .* vanilla_shapes(:,:,i);
%         rotation=get2DRotateMatrix(angles(i),first_pos);
%         controls=[vanilla_shapes(:,:,i),ones(ncpt,1)]';
%         controls=(rotation*controls)';        
%         vanilla_traj{i} = get_stk_from_bspline(controls);
%         
%         else
%         end
%     end
%     
    ...
    for i=1:n
       ...
       motor{i} = offset_stk(motor{i},offset);
       rotation=Modeler.get2DRotateMatrix(angles(i),motor{i}(1,:));
       motorAug=[motor{i},ones(size(motor{i},1),1)]';
       motor{i}=(rotation*motorAug)';
       
       controls=[motor_spline(:,:,i),ones(ncpt,1)]';
       controls=(rotation*controls)';
       %motor{i}=
       
       motor_spline(:,:,i) = controls - repmat(offset,[ncpt 1]);
    end
...
%     [ncpt,~,n] = size(vanilla_shapes);
%     
%     vanillaShapesTest=vanilla_shapes;
%     vanillaTrajTest=cell(n,1);
%     
%     sxTest=0.6;
%     syTest=2.6;
%     
%     for i=1:n;
%         vanillaTrajTest{i}=get_stk_from_bspline(vanillaShapesTest(:,:,i),50);
%     end
%     
%     for i=1:n;
%         vanillaTrajTest{i}(:,1)=sxTest.*vanillaTrajTest{i}(:,1);
%         vanillaTrajTest{i}(:,2)=syTest.*vanillaTrajTest{i}(:,2);
%     end
%     
%     % re-scale the control points
%     for i=1:n
% %        vanilla_shapes(:,:,i) = invscales(i) .* vanilla_shapes(:,:,i);
%        vanilla_shapes(:,1,i) = sxTest .* vanilla_shapes(:,1,i);
%        vanilla_shapes(:,2,i) = syTest .* vanilla_shapes(:,2,i);
%     end    
% 
%     % get trajectories from b-spline
%     vanilla_traj = cell(n,1);
%     for i=1:n
%         vanilla_traj{i} = get_stk_from_bspline(vanilla_shapes(:,:,i),50);
%     end
%     
%     
%     
